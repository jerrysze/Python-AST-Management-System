    ...
    */
    ...
    public HashMap<String, Integer> calculateOp2Nums() {
        ...
        for (String key : id2ASTModules.keySet()) {
            ASTModule module = id2ASTModules.get(key);
            int m = 0;
            ArrayList<ASTEnumOp> operators = module.getAllOperators();
            for (ASTEnumOp operator : operators) {
                String nodeType = operator.getNodeType().replace("OP_", "");
    ...
    public String mostCommonUsedOp(HashMap<String, Integer> op2Num) {
        ...
        return Collections.max(op2Num.entrySet(), Map.Entry.comparingByValue()).getKey();
    }
    ...
    public HashMap<String, Set<String>> calculateCalledFunc() {
        ...
        for (String key : id2ASTModules.keySet()) {
            ASTModule module = id2ASTModules.get(key);
            ArrayList<FunctionDefStmt> functions = module.getAllFunctions();

            for (FunctionDefStmt function : functions) {
                calledFunc.put(function.getMangle(key), new HashSet<>());
            }
            for (FunctionDefStmt function : functions) {
                String functionName = function.getMangle(key);
                ArrayList<CallExpr> calledFuncs = function.getAllCalledFunc();
                for (CallExpr callExpr : calledFuncs) {
                    String calledFuncName = callExpr.getMangle(key);
                    calledFunc.get(functionName).add(calledFuncName);
                }
            }
        }
        return calledFunc;
    ...
    public HashMap<String, Integer> calculateNode2Nums(String astID) {
        ...
        if (module != null) {
            ...
            for (ASTElement node : nodes) {
                ...
                node2Nums.put(nodeType, node2Nums.getOrDefault(nodeType, 0) + 1);
            }
        }
        return node2Nums;
    }
    ...
    public HashMap<String, Integer> processNodeFreq() {
        ...
        for (String key : id2ASTModules.keySet()) {
            ...
            for (FunctionDefStmt function : functions) {
                String functionName = function.getMangle(key);
                int numChildren = function.countChildren() + 1;
                nodeFreq.put(functionName, numChildren);
            }
        }
        return nodeFreq;
    }
